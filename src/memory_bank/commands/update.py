...
def _update_git(resolved_dir: Path, before_version: str = "", force: bool = False) -> None:
    ...
    # Reset any modified tracked files before pulling. This is a managed install
    # directory — local changes to tracked files (e.g. uv.lock regenerated by a
    # prior failed uv sync) are never intentional and cause autostash conflicts.
    dirty = subprocess.run(
        ["git", "-C", str(resolved_dir), "diff", "--name-only"],
        capture_output=True,
        text=True,
    )
    if dirty.stdout.strip():
        modified = dirty.stdout.strip().replace("\n", ", ")
        console.print(f"  [yellow]Resetting local changes to: {modified}[/yellow]")
        subprocess.run(
            ["git", "-C", str(resolved_dir), "checkout", "--", "."],
            capture_output=True,
        )
    ...
    # markers. Detect and abort before uv sync chokes on a corrupt lock file.
    ...
    if unmerged.stdout.strip():
        conflicted = unmerged.stdout.strip().replace("\n", ", ")
        raise click.ClickException(
            f"Autostash reapplication left merge conflicts in: {conflicted}\n\n"
            f"  Resolve with: git -C {resolved_dir} checkout -- .\n"
